...
import { GenerateWatermark } from "../cwm";
...
function bindMutationEvent(
  ...
  return {
    ...
    targetObserver: {
      disconnect: () =>
        eventList.map(eventName => target.removeEventListener(eventName, () => callback(), false))
    }
  };
}
...
export const observer = (
  ...
  const containerObserver: MutationObserver = new mutationObserver(
